...
public class MapTerrain : MonoBehaviour
{
    ...
    static string[] mapNameList = new string[] { "1st", "5", "9", "Hylym" };

    static Dictionary<string, int[,]> mapList = new Dictionary<string, int[,]>()
    {
        { mapNameList[0], map1st } ,
        { mapNameList[1], map5 },
        { mapNameList[2], map9 },
        { mapNameList[3], mapHylym }
    };

    public static string mapName = mapNameList[1];
    public int[,] map;
    ...
    void Awake()
    {
        ...
        switch (mapChoice)
        {
            case "1st" : mapImage.sprite = MapSprites[0]; break;
            case "5": mapImage.sprite = MapSprites[1]; break;
            case "9": mapImage.sprite = MapSprites[2]; break;
            case "Hylym": mapImage.sprite = MapSprites[3]; break;
        }
    }
}
